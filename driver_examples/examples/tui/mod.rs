...
impl SledTerminalDisplay {
    ...
    pub fn set_leds(&mut self, leds: impl Iterator<Item = (Rgb, Vec2)>) {
...
fn draw_led(ctx: &mut Context, led: &(Rgb, Vec2)) {
    let (col, pos) = led;
    ctx.draw(&Circle {
        x: pos.x as f64,
        y: pos.y as f64,
...
#[allow(dead_code)]
fn main() -> io::Result<()> {
    let sled = Sled::<Rgb>::new("./examples/resources/config.yap").unwrap();
